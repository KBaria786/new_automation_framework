...
/**
 * Utility class containing Selenium methods braced with exception handling,
...
public class WebDriverController {
	...
	/**
	 * Creates a new instance of WebDriverController.
	 * 
	 * @param webDriver web driver instance to be used by the Selenium utility
	 *                  methods
	 */
	...
	public boolean get(String stepDescription, String url) {
		if (StringUtils.isNotBlank(url)) {
			try {
				...
				// reporting
				reportStepSuccess(stepDescription, String.format("Successfully loaded url: %s", url),
						getSuccessScreenshot());
			} catch (Exception e) {
				// logging
				log(Level.ERROR, stepDescription, String.format("Exception occurred while loading url: %s", url), e);
				// reporting
				reportStepFailure(stepDescription, String.format("Exception occurred while loading url: %s", url), e,
			...
			// reporting
			reportStepFailure(stepDescription, "Null or blank url", getFailureScreenshot());
		}
		return false;
	}
	...
	public boolean close(String stepDescription) {
		try {
			...
			// reporting
			reportStepSuccess(stepDescription, "Successfully closed current window", getSuccessScreenshot());
		} catch (Exception e) {
			// logging
	...
	/**
	 * Click on the web element found using the given 'locatorString' string which
	 * is visible and within viewport.
	 * 
	 * @param stepDescription short step description
	...
	public boolean click(String stepDescription, String locatorString) {
		if (StringUtils.isNotBlank(locatorString)) {
			WebElement webElement = waitForClickabilityOfElement(stepDescription, locatorString,
					defaultExplicitWaitDuration);
	...
	/**
	 * Click on the web element found using the given 'by' locator which is visible
	...
	private boolean performClickOperation(String stepDescription, WebElement webElement) {
		if (webElement != null) {
			try {
				...
				// reporting
				reportStepSuccess(stepDescription, "Successfully clicked web element", getSuccessScreenshot());
				return true;
			} catch (Exception e) {
				// logging
				log(Level.ERROR, stepDescription, "Exception occurred while clicking web element", e);
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while clicking web element", e,
						getFailureScreenshot());
			}
		}
		return false;
	...
	/**
	 * Click on the web element using {@link Actions} found using the given
	 * 'locatorString' string which is visible and within the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	 * @return true if operation is successful otherwise false
	...
	/**
	 * Click on the web element using {@link Actions} found using the given 'by'
	 * locator which is visible and within the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param by              the locator to find the web element with
	 * @return true if operation is successful otherwise false
	...
	/**
	 * Click on the web element using {@link Actions} which is visible and within
	 * the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param webElement      the web element to click on
	...
	/**
	 * Click on the web element using {@link JavascriptExecutor} found using the
	 * given 'locatorString' string that is present on the DOM.
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	 * @return true if operation is successful otherwise false
	...
	public boolean clickUsingJSExecutor(String stepDescription, String locatorString) {
		if (StringUtils.isNotBlank(locatorString)) {
			WebElement webElement = waitForPresenceOfElement(stepDescription, locatorString,
					defaultExplicitWaitDuration);
	...
	/**
	 * Click on the first web using {@link JavascriptExecutor} element found using
	 * the given 'by' locator that is present on the DOM.
	 * 
	 * @param stepDescription short step description
	 * @param by              the locator to find the web element with
	 * @return true if operation is successful otherwise false
	...
	/**
	 * Click on the web element using {@link JavascriptExecutor} which is present on
	 * the DOM.
	 * 
	 * @param stepDescription short step description
	 * @param webElement      the web element to click on
...
//				webElement = waitForVisibilityOfElement(stepDescription, webElement, defaultExplicitWaitDuration);
	...
	/**
	 * Method to simulate typing into the web element found using the given
	 * 'locatorString' string which is visible and within the viewport. This may
	 * also set the web element's value.
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	...
	public boolean sendKeys(String stepDescription, String locatorString, String value) {
		if (StringUtils.isNotBlank(locatorString) && StringUtils.isNotBlank(value)) {
			WebElement webElement = waitForVisibilityOfElement(stepDescription, locatorString,
					defaultExplicitWaitDuration);
			return performSendKeysOperation(stepDescription, webElement, value);
		} else {
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. locatorString: {}, value: {}", locatorString,
					value);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is null or blank. locatorString: %s, value: %s",
							locatorString, value),
					null);
		}
		return false;
	...
	/**
	 * Method to simulate typing into the web element found using the given 'by'
	 * locator which is visible and within the viewport. This may also set the web
	 * element's value.
	 * 
	 * @param stepDescription short step description
	...
	public boolean sendKeys(String stepDescription, By by, String value) {
		if (by != null && StringUtils.isNotBlank(value)) {
			...
			// logging
			log(Level.ERROR, stepDescription, "One or more of the required fields is null or blank. by: {}, value: {}",
					by, value);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is null or blank. by: %s, value: %s", by, value),
					null);
		}
		return false;
	}
	...
	/**
	 * Method to simulate typing into the web element which is visible and within
	 * the viewport. This may also set the web element's value.
	 * 
	 * @param stepDescription short step description
	...
	public boolean sendKeys(String stepDescription, WebElement webElement, String value) {
		if (webElement != null && StringUtils.isNotBlank(value)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. webElement: {}, value: {}", webElement,
					value);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is null or blank. webElement: %s, value: %s",
							webElement, value),
					null);
		}
		return false;
	...
	private boolean performSendKeysOperation(String stepDescription, WebElement webElement, String value) {
		if (webElement != null && StringUtils.isNotBlank(value)) {
			try {
				...
				// reporting
				reportStepSuccess(stepDescription, String.format("Successfully sent keys: %s to web element", value),
						null);
				return true;
			} catch (Exception e) {
				// logging
				...
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while sending keys to web element", e,
						getScreenshot());
			}
		}
		return false;
	...
	/**
	 * Method to simulate clearing the text and then typing into the web element
	 * found using the given 'locatorString' string which is visible and within the
	 * viewport. This may also set the web element's value.
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	 * @param value           character sequence to send to the web element
	...
	/**
	 * Method to simulate clearing the text and then typing into the web element
	 * found using the given 'by' locator which is visible and within the viewport.
	 * This may also set the web element's value.
	 * 
	 * @param stepDescription short step description
	 * @param by              the locator to find the element with
	...
	/**
	 * Method to simulate clearing the text and then typing into the web element
	 * which is visible and within the viewport. This may also set the web element's
	 * value.
	 * 
	 * @param stepDescription short step description
	 * @param webElement      the web element to send keys to
	...
	/**
	 * Select all options that display text matching the given 'visibleText' for the
	 * web element found using the given 'locatorString' string which is visible and
	 * within the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	...
	public boolean selectByVisibleText(String stepDescription, String locatorString, String visibleText) {
		if (StringUtils.isNotBlank(locatorString) && StringUtils.isNotBlank(visibleText)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. locatorString: {}, visibleText: {}",
					locatorString, visibleText);
			// reporting
			reportStepFailure(stepDescription,
					String.format(
							"One or more of the required fields is null or blank. locatorString: %s, visibleText: %s",
							locatorString, visibleText),
					null);
		}
		return false;
	...
	/**
	 * Select all options that display text matching the given 'visibleText' string
	 * for the web element found using the given 'by' locator which is visible and
	 * within the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param by              the locator to find the element with
	...
	public boolean selectByVisibleText(String stepDescription, By by, String visibleText) {
		if (by != null && StringUtils.isNotBlank(visibleText)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. by: {}, visibleText: {}", by, visibleText);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is null or blank. by: %s, visibleText: %s", by,
							visibleText),
					null);
		}
		return false;
	...
	/**
	 * Select all options that display text matching the given 'visibleText' string
	 * for the web element which is visible and within the viewport.
	 * 
	 * @param stepDescription short step description
	...
	public boolean selectByVisibleText(String stepDescription, WebElement webElement, String visibleText) {
		if (webElement != null && StringUtils.isNotBlank(visibleText)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. webElement: {}, visibleText: {}", webElement,
					visibleText);
			// reporting
			reportStepFailure(stepDescription,
					String.format(
							"One or more of the required fields is null or blank. webElement: %s, visibleText: %s}",
	 ...
	 * @return true if operation is successful otherwise false
	 */
	private boolean performSelectByVisibleTextOperation(String stepDescription, WebElement webElement,
			String visibleText) {
		if (webElement != null && StringUtils.isNotBlank(visibleText)) {
			try {
				...
				// logging
				log(Level.INFO, stepDescription, "Successfully selected option: {} by visible text on web element",
						visibleText, webElement);
				// reporting
				reportStepSuccess(stepDescription,
						String.format("Successfully selected option: %s by visible text on web element", visibleText),
						null);
				return true;
			} catch (Exception e) {
				// logging
				...
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while selecting option by visible text", e,
						getScreenshot());
			}
		}
		return false;
	...
	/**
	 * Select all options that have a value matching the given 'value' string for
	 * the web element found using the given 'locatorString' string which is visible
	 * and within the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	...
	public boolean selectByValue(String stepDescription, String locatorString, String value) {
		if (StringUtils.isNotBlank(locatorString) && StringUtils.isNotBlank(value)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is blank. locatorString: {}, value: {}", locatorString, value);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is blank. locatorString: %s, value: %s",
							locatorString, value),
					null);
		}
		return false;
	...
	/**
	 * Select all options that have a value matching the given 'value' string for
	 * the web element which is visible and within the viewport.
	 * 
	 * @param stepDescription short step description
	 * @param webElement      the web element to select options for
	 * @param value           the value to match against
	...
	private boolean performSelectByValueOperation(String stepDescription, WebElement webElement, String value) {
		if (webElement != null && StringUtils.isNotBlank(value)) {
			try {
				...
				// logging
				log(Level.INFO, stepDescription, "Successfully selected option: {} by value on web element", value,
						webElement);
				// reporting
				reportStepSuccess(stepDescription,
						String.format("Successfully selected option: %s by value on web element", value), null);
				return true;
			} catch (Exception e) {
				// logging
				log(Level.ERROR, stepDescription, "Exception occurred while selecting option by value", e);
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while selecting option by value", e,
						getFailureScreenshot());
			}
		}
		return false;
	...
	/**
	 * Method to drag and drop a web element which is visible and within the
	 * viewport.
	 * 
	 * @param stepDescription     short step description
	...
	public boolean dragAndDrop(String stepDescription, String sourceLocatorString, String targetLocatorString) {
		if (StringUtils.isNotBlank(sourceLocatorString) && StringUtils.isNotBlank(targetLocatorString)) {
			WebElement sourceWebElement = waitForVisibilityOfElement(stepDescription, sourceLocatorString,
					defaultExplicitWaitDuration);
			WebElement targetWebElement = waitForVisibilityOfElement(stepDescription, targetLocatorString,
					defaultExplicitWaitDuration);
			return performDragAndDropOperation(stepDescription, sourceWebElement, targetWebElement);
		} else {
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is blank. sourceLocatorString: {}, targetLocatorString: {}",
					sourceLocatorString, targetLocatorString);
			// reporting
			reportStepFailure(stepDescription, String.format(
					"One or more of the required fields is blank. sourceLocatorString: %s, targetLocatorString: %s",
					sourceLocatorString, targetLocatorString), null);
		}
		return false;
	}
	...
	public boolean dragAndDrop(String stepDescription, By sourceBy, By targetBy) {
		if (sourceBy != null && targetBy != null) {
			WebElement sourceWebElement = waitForVisibilityOfElement(stepDescription, sourceBy,
					defaultExplicitWaitDuration);
			WebElement targetWebElement = waitForVisibilityOfElement(stepDescription, targetBy,
					defaultExplicitWaitDuration);
			return performDragAndDropOperation(stepDescription, sourceWebElement, targetWebElement);
		} else {
			// logging
			log(Level.ERROR, stepDescription, "One or more of the required fields is null. sourceBy: {}, targetBy: {}",
					sourceBy, targetBy);
			// reporting
			reportStepFailure(stepDescription, String.format(stepDescription,
					"One or more of the required fields is null. sourceBy: %s, targetBy: %s", sourceBy, targetBy),
					null);
		}
		return false;
	}
	 ...
	 * @param targetWebElement the web element to drag and drop the given
	 *                         'sourceWebElement' to
	 * @return true if operation is successful otherwise false
	 */
	public boolean dragAndDrop(String stepDescription, WebElement sourceWebElement, WebElement targetWebElement) {
		if (sourceWebElement != null && targetWebElement != null) {
			sourceWebElement = waitForVisibilityOfElement(stepDescription, sourceWebElement,
					defaultExplicitWaitDuration);
			targetWebElement = waitForVisibilityOfElement(stepDescription, targetWebElement,
					defaultExplicitWaitDuration);
			return performDragAndDropOperation(stepDescription, sourceWebElement, targetWebElement);
		} else {
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null. sourceWebElement: {}, targetWebElement: {}",
					sourceWebElement, targetWebElement);
			// reporting
			reportStepFailure(stepDescription,
					String.format(
							"One or more of the required fields is null. sourceWebElement: %s, targetWebElement: %s",
							sourceWebElement, targetWebElement),
					null);
		}
		return false;
	 ...
	 * @return true if operation is successful otherwise false
	 */
	private boolean performDragAndDropOperation(String stepDescription, WebElement sourceWebElement,
			WebElement targetWebElement) {
		if (sourceWebElement != null && targetWebElement != null) {
			try {
				Actions actions = new Actions(webDriver);
				actions.moveToElement(sourceWebElement).clickAndHold().moveToElement(targetWebElement).release().build()
						.perform();
				// logging
				...
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while dragging and dropping element", e,
						getFailureScreenshot());
			}
		}
		return false;
	...
	public boolean scrollToElement(String stepDescription, String locatorString, String scrollOptions) {
		if (StringUtils.isNotBlank(locatorString)) {
			WebElement webElement = waitForPresenceOfElement(stepDescription, locatorString,
					defaultExplicitWaitDuration);
			return performScrollToElementOperation(stepDescription, webElement, scrollOptions);
		} else {
			// logging
			log(Level.ERROR, stepDescription, "Null or blank locator string. locatorString: {}", locatorString);
			// reporting
			reportStepFailure(stepDescription,
					String.format("Null or blank locator string. locatorString: %s", locatorString), null);
		}
		return false;
	}
	 ...
	 * @param scrollOptions   scroll options to use
	 * @return true if operation is successful otherwise false
	 */
	public boolean scrollToElement(String stepDescription, By by, String scrollOptions) {
		if (by != null) {
			...
			// logging
			log(Level.ERROR, stepDescription, "Null locator. by: {}", by);
			// reporting
			reportStepFailure(stepDescription, String.format("Null locator. by: %s", by), null);
		}
		return false;
	}
	 ...
	 * 
	 * @param stepDescription short step description
	 * @param webElement      web element to scroll to
	 * @param scrollOptions   scroll options to use
	 * @return true if operation is successful otherwise false
	 */
	public boolean scrollToElement(String stepDescription, WebElement webElement, String scrollOptions) {
		if (webElement != null) {
			return performScrollToElementOperation(stepDescription, webElement, scrollOptions);
		} else {
			// logging
			log(Level.ERROR, stepDescription, "Null web element. webElement: {}", webElement);
			// reporting
			reportStepFailure(stepDescription, String.format("Null web element. webElement: %s", webElement), null);
		}
		return false;
	}
	 ...
	 * @return true if operation is successful otherwise false
	 */
	private boolean performScrollToElementOperation(String stepDescription, WebElement webElement,
		...
		if (webElement != null) {
			try {
				...
				// logging
				log(Level.INFO, stepDescription, "Successfully scrolled to web element: {}", webElement);
				// reporting
				reportStepSuccess(stepDescription, "Successfully scrolled to web element", getSuccessScreenshot());
				return true;
			} catch (Exception e) {
				// logging
				log(Level.ERROR, stepDescription, "Exception occurred while scrolling element", e);
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while scrolling element", e,
						getFailureScreenshot());
			}
		}
		return false;
	...
	/**
	 * Get the value of the given attribute of the element which is present on the
	 * DOM.
	 * 
	 * @param stepDescription short step description
	...
	public String getAttribute(String stepDescription, String locatorString, String attribute) {
		if (StringUtils.isNotBlank(locatorString) && StringUtils.isNotBlank(attribute)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is blank. locatorString: {}, attribute: {}", locatorString,
					attribute);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is blank. locatorString: %s, attribute: %s",
							locatorString, attribute),
					null);
		}
		return null;
	}
	...
	public String getAttribute(String stepDescription, By by, String attribute) {
		if (by != null && StringUtils.isNotBlank(attribute)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. by: {}, attribute: {}", by, attribute);
			// reporting
			reportStepFailure(stepDescription, String.format(
					"One or more of the required fields is null or blank. by: %s, attribute: %s", by, attribute), null);
		}
		return null;
	}
	...
	public String getAttribute(String stepDescription, WebElement webElement, String attribute) {
		if (webElement != null && StringUtils.isNotBlank(attribute)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. webElement: {}, attribute: {}", webElement,
					attribute);
			// reporting
			reportStepFailure(stepDescription,
					String.format("One or more of the required fields is null or blank. webElement: %s, attribute: %s",
	...
	private String performGetAttributeOperation(String stepDescription, WebElement webElement, String attribute) {
		if (webElement != null && StringUtils.isNotBlank(attribute)) {
			try {
				...
				// logging
				log(Level.INFO, stepDescription, "Successfully got attribute: {} of web element", attribute,
						webElement);
				// reporting
				...
				// reporting
				reportStepFailure(stepDescription, "Exception occurred while getting attribute of web element", e,
						getScreenshot());
			}
		}
		return null;
	 ...
	 * 
	 * @param stepDescription short step description
	 * @param windowType      The type of new browser window to be created. The
	 *                        created window is not guaranteed to be of the
	 *                        requested type; if the driver does not support the
	 *                        requested type, a new browser window will be created
	 *                        of whatever type the driver does support.
		...
		if (isAlertPresent(stepDescription)) {
			if (StringUtils.isNotBlank(value)) {
				try {
					...
					// reporting
					reportStepSuccess(stepDescription, String.format("Successfully sent keys: %s to alert", value),
							null);
					return true;
				} catch (Exception e) {
					// logging
	...
	/**
	 * Select a frame by its (zero-based) index. Once the frame has been selected,
	...
	public boolean switchToIFrameByIndex(String stepDescription, int index) {
		if (index > 0) {
			try {
				...
				// reporting
				reportStepSuccess(stepDescription,
						String.format("Successfully switched to iframe with index: %s", index), null);
				return true;
			} catch (Exception e) {
				// logging
	 ...
	 *                        frame element, or the (zero-based) index
	 * @return true if driver is focused on the given frame otherwise false
	 */
	 ...
	 * 
	 * @param stepDescription short step description
	 * @return true if driver is focused on the top window/first frame otherwise
	 *         false
	 */
	public boolean switchToDefaultContent(String stepDescription) {
		try {
			...
			// reporting
			reportStepFailure(stepDescription, "Exception occurred while switching to default content", e,
					getScreenshot());
		}
		return false;
	}
	...
	/**
	 * Capture screenshot for successful steps.
	 * 
	 * @return Obtain the screenshot as raw bytes.
	 */
	private byte[] getSuccessScreenshot() {
	 ...
	 * @return Obtain the screenshot as raw bytes.
	 */
	private byte[] getFailureScreenshot() {
	 ...
	 * This does not necessarily mean that the element is visible.
	 * <p>
	 * This method generates report if the element is not present on the DOM.
	 * </p>
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the web element with
	 ...
	 * This does not necessarily mean that the element is visible.
	 * <p>
	 * This method generates report if the element is not present on the DOM based
	 * on the value of 'generateReport'
	 * </p>
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the web element with
	 * @param duration        the duration to wait for
	 * @param generateReport  boolean value to determine if reports will be
	 *                        generated or not
	 * @return the web element once it is located
	 */
	private WebElement waitForPresenceOfElement(String stepDescription, String locatorString, Duration duration,
			boolean generateReport) {
		if (StringUtils.isNotBlank(locatorString)) {
			List<By> locators = getLocators(locatorString);
			Optional<WebElement> webElementOptional = locators.stream()
					.map(locator -> waitForPresenceOfElement(stepDescription, locator,
							duration != null ? duration : defaultExplicitWaitDuration, false))
					.filter(webElement -> webElement != null).findFirst();
			if (webElementOptional.isPresent()) {
				...
				if (generateReport) {
					Exception e = new NoSuchElementException(
							String.format("No element present by locator string: %s", locatorString));
	 ...
	 * @return the web element once it is located
	 */
	private WebElement waitForPresenceOfElement(String stepDescription, By by, Duration duration,
			boolean generateReport) {
		if (by != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
						duration != null ? duration : defaultExplicitWaitDuration);
				...
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription, "Exception occurred while waiting for presence of element", e,
							getScreenshot());
				}
			}
		} else {
	 ...
	 * has a height and width that is greater than 0.
	 * <p>
	 * This method generates report if the element is not visible on the DOM.
	 * </p>
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the element with
	 ...
	 * has a height and width that is greater than 0.
	 * <p>
	 * This method generates report if the element is not visible on the DOM based
	 * on the value of 'generateReport'
	 * </p>
	 * 
	 * @param stepDescription short step description
	 ...
	 * @return the web element once it is located and visible
	 */
	private WebElement waitForVisibilityOfElement(String stepDescription, String locatorString, Duration duration,
			boolean generateReport) {
		if (StringUtils.isNotBlank(locatorString)) {
			Optional<WebElement> webElementOptional = getLocators(locatorString).stream()
					.map(locator -> waitForVisibilityOfElement(stepDescription, locator,
							duration != null ? duration : defaultExplicitWaitDuration, false))
					.filter(webElement -> webElement != null).findFirst();
			if (webElementOptional.isPresent()) {
				return webElementOptional.get();
				...
				if (generateReport) {
					Exception e = new NoSuchElementException(
							String.format("No visible element found by locator string: %s", locatorString));
					reportStepFailure(stepDescription, e.getMessage(), e, getScreenshot());
				}
			}
		} else {
	 ...
	 * @return the web element once it is located and visible
	 */
	private WebElement waitForVisibilityOfElement(String stepDescription, By by, Duration duration,
			boolean generateReport) {
		if (by != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
				...
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription, "Exception occurred while waiting for visibility of element", e,
							getScreenshot());
				}
			}
		} else {
	 ...
	 * @return the web element once it is located and visible
	 */
	private WebElement waitForVisibilityOfElement(String stepDescription, WebElement webElement, Duration duration,
			boolean generateReport) {
		if (webElement != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
	 ...
	 * present on the DOM.
	 * <p>
	 * This method generates report if the element is visible on the DOM.
	 * </p>
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   string to find the web element with
	 ...
	 * present on the DOM.
	 * <p>
	 * This method generates report if the element is visible on the DOM based on
	 ...
	 *         stale element
	 */
	private boolean waitForInvisibilityOfElement(String stepDescription, String locatorString, Duration duration,
			boolean generateReport) {
		if (StringUtils.isNotBlank(locatorString)) {
			Optional<By> locatorOptional = getLocators(locatorString).stream()
					.filter(locator -> waitForInvisibilityOfElement(stepDescription, locator,
							duration != null ? duration : defaultExplicitWaitDuration, false))
					.findFirst();
			if (locatorOptional.isEmpty()) {
				...
				if (generateReport) {
					Exception e = new NoSuchElementException(
							String.format("No invisible element found by locator string: %s", locatorString));
					reportStepFailure(stepDescription, e.getMessage(), e, getScreenshot());
				}
			}
	 ...
	 *         stale element
	 */
	private boolean waitForInvisibilityOfElement(String stepDescription, By by, Duration duration,
			boolean generateReport) {
		if (by != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
				...
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription, "Exception occurred while waiting for invisibility of element",
							e, getScreenshot());
				}
			}
		} else {
	 ...
	 *         stale element
	 */
	private boolean waitForInvisibilityOfElement(String stepDescription, WebElement webElement, Duration duration,
			boolean generateReport) {
		if (webElement != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
	 ...
	 * @return the web element once it is located and visible
	 */
	private WebElement waitForClickabilityOfElement(String stepDescription, String locatorString, Duration duration,
			boolean generateReport) {
		if (StringUtils.isNotBlank(locatorString)) {
			Optional<WebElement> webElementOptional = getLocators(locatorString).stream()
					.map(locator -> waitForClickabilityOfElement(stepDescription, locator,
							duration != null ? duration : defaultExplicitWaitDuration, false))
					.filter(webElement -> webElement != null).findFirst();
			if (webElementOptional.isPresent()) {
				return webElementOptional.get();
				...
				if (generateReport) {
					Exception e = new NoSuchElementException(
							String.format("No clickable element found by locator string: %s", locatorString));
					reportStepFailure(stepDescription, e.getMessage(), e, getScreenshot());
				}
			}
		} else {
	 ...
	 * @return the web element once it is located and visible
	 */
	private WebElement waitForClickabilityOfElement(String stepDescription, By by, Duration duration,
			boolean generateReport) {
		if (by != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
				...
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription, "Exception occurred while waiting for clickability of element",
							e, getScreenshot());
				}
			}
		} else {
	 ...
	 * @return the web element once it is located and visible
	 */
	private WebElement waitForClickabilityOfElement(String stepDescription, WebElement webElement, Duration duration,
			boolean generateReport) {
		if (webElement != null) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
	 ...
	 */
	public boolean isElementAttributeEqualTo(String stepDescription, String locatorString, String attribute,
	 ...
	 * value.
	 * <p>
	 * This method generates report if the element attribute does not equal to the
	 * given 'value'.
	 * </p>
	 * 
	 * @param stepDescription short step description
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	public boolean waitForAttributeToBe(String stepDescription, String locatorString, String attribute, String value,
			Duration duration) {
	 ...
	 * <p>
	 * This method generates report if the element attribute does not equal to the
	 * given 'value' based on the DOM based on the value of 'generateReport'.
	 * </p>
	 * 
	 * @param stepDescription short step description
	 * @param locatorString   the string to find the web element with
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	private boolean waitForAttributeToBe(String stepDescription, String locatorString, String attribute, String value,
			Duration duration, boolean generateReport) {
		if (StringUtils.isNotBlank(locatorString) && StringUtils.isNotBlank(attribute)
				&& StringUtils.isNotBlank(value)) {
			Optional<By> locatorOptional = getLocators(locatorString).stream()
					.filter(locator -> waitForAttributeToBe(stepDescription, locator, attribute, value,
							duration != null ? duration : defaultExplicitWaitDuration, false))
					.findFirst();
			if (locatorOptional.isEmpty()) {
				// logging
				log(Level.ERROR, stepDescription, "No element found with {} = {} by locator string: {}", attribute,
						value, locatorString);
				// reporting
				if (generateReport) {
					Exception e = new NoSuchElementException(String.format(
							"No element found with %s = %s by locator string: %s", attribute, value, locatorString));
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is blank. locatorString: {}, attribute: {}, value: {}",
					locatorString, attribute, value);
			// reporting
			if (generateReport) {
				reportStepFailure(stepDescription, String.format(
						"One or more of the required fields is blank. locatorString: %s, attribute: %s, value: %s",
						locatorString, attribute, value), null);
			}
		}
		return false;
	}
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	public boolean isElementAttributeEqualTo(String stepDescription, By by, String attribute, String value,
			Duration duration) {
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	public boolean waitForAttributeToBe(String stepDescription, By by, String attribute, String value,
			Duration duration) {
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	private boolean waitForAttributeToBe(String stepDescription, By by, String attribute, String value,
			Duration duration, boolean generateReport) {
		if (by != null && StringUtils.isNotBlank(attribute) && StringUtils.isNotBlank(value)) {
			try {
				...
				// logging
				log(Level.ERROR, stepDescription,
						String.format("Exception occurred while waiting for attribute %s to be %s", attribute, value),
						e);
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription, String
							.format("Exception occurred while waiting for attribute %s to be %s", attribute, value), e,
							getScreenshot());
				}
			}
		} else {
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. by: {}, attribute: {}, value: {}", by,
					attribute, value);
			// reporting
			if (generateReport) {
				reportStepFailure(stepDescription,
						String.format(
								"One or more of the required fields is null or blank. by: %s, attribute: %s, value: %s",
								by, attribute, value),
						getScreenshot());
			}
		}
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	public boolean isElementAttributeEqualTo(String stepDescription, WebElement webElement, String attribute,
			String value, Duration duration) {
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	public boolean waitForAttributeToBe(String stepDescription, WebElement webElement, String attribute, String value,
			Duration duration) {
	 ...
	 * @return true when element has css or html attribute with the value
	 */
	private boolean waitForAttributeToBe(String stepDescription, WebElement webElement, String attribute, String value,
			Duration duration, boolean generateReport) {
		if (webElement != null && StringUtils.isNotBlank(attribute) && StringUtils.isNotBlank(value)) {
			...
			// logging
			log(Level.ERROR, stepDescription,
					"One or more of the required fields is null or blank. webElement: {}, attribute: {}, value: {}",
					webElement, attribute, value);
			// reporting
			if (generateReport) {
				reportStepFailure(stepDescription, String.format(
						"One or more of the required fields is null or blank. webElement: %s, attribute: %s, value: %s",
						webElement, attribute, value), getScreenshot());
			}
		}
		return false;
	}
	 ...
	 */
	public boolean doesElementAttributeContain(String stepDescription, String locatorString, String attribute,
			String value, Duration duration) {
	 ...
	 * a specific value.
	 * <p>
	 * This method generates report if the element attribute does not contain the
	 * given 'value'.
	 * </p>
	 * 
	 * @param stepDescription short step description
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	public boolean waitForAttributeToContain(String stepDescription, String locatorString, String attribute,
			String value, Duration duration) {
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	private boolean waitForAttributeToContain(String stepDescription, String locatorString, String attribute,
			String value, Duration duration, boolean generateReport) {
		if (StringUtils.isNotBlank(locatorString) && StringUtils.isNotBlank(attribute)
				&& StringUtils.isNotBlank(value)) {
			Optional<By> locatorOptional = getLocators(locatorString).stream()
					.filter(locator -> waitForAttributeToContain(stepDescription, locator, attribute, value,
							duration != null ? duration : defaultExplicitWaitDuration, false))
					.findFirst();
			if (locatorOptional.isEmpty()) {
				// logging
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	public boolean doesElementAttributeContain(String stepDescription, By by, String attribute, String value,
			Duration duration) {
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	public boolean waitForAttributeToContain(String stepDescription, By by, String attribute, String value,
			Duration duration) {
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	private boolean waitForAttributeToContain(String stepDescription, By by, String attribute, String value,
			Duration duration, boolean generateReport) {
		if (by != null && StringUtils.isNotBlank(attribute) && StringUtils.isNotBlank(value)) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	public boolean doesElementAttributeContain(String stepDescription, WebElement webElement, String attribute,
			String value, Duration duration) {
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	public boolean waitForAttributeToContain(String stepDescription, WebElement webElement, String attribute,
			String value, Duration duration) {
	 ...
	 * @return true when element has css or html attribute which contains the value
	 */
	private boolean waitForAttributeToContain(String stepDescription, WebElement webElement, String attribute,
			String value, Duration duration, boolean generateReport) {
		if (webElement != null && StringUtils.isNotBlank(attribute) && StringUtils.isNotBlank(value)) {
			try {
				WebDriverWait wait = new WebDriverWait(webDriver,
				...
				// logging
				log(Level.ERROR, stepDescription, String.format(
						"Exception occurred while waiting for attribute %s to contain %s", attribute, value), e);
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription,
							String.format("Exception occurred while waiting for attribute %s to contain %s}", attribute,
									value),
							e, getScreenshot());
				}
			}
			...
			if (generateReport) {
				reportStepFailure(stepDescription, String.format(
						"One or more of the required fields is null or blank. webElement: %s, attribute: %s, value: %s",
						webElement, attribute, value), null);
			}
		}
		return false;
	}
	 ...
	 */
	public <T> T waitForCustomCondition(String stepDescription, ExpectedCondition<T> expectedCondition,
			Duration duration, boolean generateReport) {
		if (expectedCondition != null) {
			try {
				...
				// reporting
				if (generateReport) {
					reportStepFailure(stepDescription, "Exception occurred while waiting for custom expected condition",
							e, getScreenshot());
				}
			}
		} else {
	...
	public WebElement findElement(String locatorString) {
		if (StringUtils.isNotBlank(locatorString)) {
			List<By> locators = getLocators(locatorString);
			Optional<WebElement> webElementOptional = locators.stream().map(locator -> tryFindingElement(locator))
					.filter(webElement -> webElement != null).findFirst();
			if (webElementOptional.isPresent()) {
				return webElementOptional.get();
			} else {
				...
				// reporting
				Exception e = new NoSuchElementException(
						String.format("No web element found with locator string: %s", locatorString));
	...
	/**
	 * Find all elements within the current page using the given 'locatorString'
	 * locator.
	 * 
	 * @param locatorString the string to find the web element with
	...
	public List<WebElement> findElements(String locatorString) {
		if (StringUtils.isNotBlank(locatorString)) {
			...
			Optional<List<WebElement>> webElementListOptional = locators.stream()
					.map(locator -> tryFindingWebElements(locator)).filter(webElements -> !webElements.isEmpty())
					.findFirst();
			if (webElementListOptional.isPresent()) {
				...
				// reporting
				Exception e = new NoSuchElementException(
						String.format("No web elements found with locator string: %s", locatorString));
	...
	public List<WebElement> findElements(By by) {
		if (by != null) {
			try {
				...
				log(Level.ERROR, "Find elements", "Exception occurred while finding web elements with locator", e);
				// reporting
				reportStepFailure("Find elements", "Exception occurred while finding web elements with locator", e,
						getScreenshot());
			}
		} else {
			// logging
	...
	/**
	 * Find the first child web element of the given 'parentWebElement' web element
	 * using the given 'locatorString' string.
	 * 
	 * @param parentWebElement parent web element from which to locate the child web
	 *                         element
	...
	public WebElement findChildWebElement(WebElement parentWebElement, String locatorString) {
		if (parentWebElement != null && StringUtils.isNotBlank(locatorString)) {
			...
			if (webElementOptional.isPresent()) {
				...
				// logging
				log(Level.ERROR, "Find child web element", "No child web element found with locator string: {}",
						locatorString);
				// reporting
				Exception e = new NoSuchElementException(
						String.format("No child web element found with locator string: %s}", locatorString));
				reportStepFailure("Find child web element", e.getMessage(), e, getScreenshot());
			}
		} else {
			// logging
			log(Level.ERROR, "Find child web element",
					"One or more of the required fields is null or blank. parentWebElement: {}, locatorString: {}",
					parentWebElement, locatorString);
			// reporting
			reportStepFailure("Find child web element", String.format(
					"One or more of the required fields is null or blank. parentWebElement: %s, locatorString: %s",
					parentWebElement, locatorString), null);
		}
		return null;
	}
	...
	private WebElement tryFindingChildWebElement(WebElement parentWebElement, By by) {
		if (parentWebElement != null && by != null) {
			...
			// logging
			log(Level.ERROR, null, "One or more of the required fields is null. parentWebElement: {}, by: {}",
					parentWebElement, by);
		}
		return null;
	}
	...
	public WebElement findChildWebElement(WebElement parentWebElement, By by) {
		if (parentWebElement != null && by != null) {
			try {
				...
				// reporting
				reportStepFailure("Find child web element", "Exception occurred while finding child web elements", e,
						getScreenshot());
			}
		} else {
			// logging
			log(Level.ERROR, "Find child web element",
					"One or more of the required fields is null. parentWebElement: {}, by: {}", parentWebElement, by);
			// reporting
			reportStepFailure("Find child web element",
					String.format("One or more of the required fields is null. parentWebElement: %s, by: %s",
							parentWebElement, by),
					null);
		}
		return null;
	...
	/**
	 * Find list of child web elements of the given 'parentWebElement' web element
	 * using the given 'by' locator
	 * 
	 * @param parentWebElement parent web element from which to locate the child web
	 *                         element
	...
	public List<WebElement> findChildWebElements(WebElement parentWebElement, String locatorString) {
		if (parentWebElement != null && StringUtils.isNotBlank(locatorString)) {
			...
			Optional<List<WebElement>> webElementListOptional = locators.stream()
					...
					.filter(webElements -> !webElements.isEmpty()).findFirst();
			if (webElementListOptional.isPresent()) {
				return webElementListOptional.get();
			} else {
				// logging
				log(Level.ERROR, "Find child web elements", "No child web elements found with locator string: {}",
						locatorString);
				// reporting
				Exception e = new NoSuchElementException(
						String.format("No child web elements found with locator string: %s", locatorString));
				reportStepFailure("Find child web elements", e.getMessage(), e, getScreenshot());
			}
		} else {
			// logging
			log(Level.ERROR, "Find child web elements",
					"One or more of the required fields is null or blank. parentWebElement: {}, locatorString: {}",
					parentWebElement, locatorString);
			// reporting
			reportStepFailure("Find child web elements", String.format(
					"One or more of the required fields is null or blank. parentWebElement: %s, locatorString: %s}",
					parentWebElement, locatorString), null);
		}
		return List.of();
	...
	public List<WebElement> findChildWebElements(WebElement parentWebElement, By by) {
		if (parentWebElement != null && by != null) {
			try {
				...
				// reporting
				reportStepFailure("Find child web elements", "Exception occurred while finding child web elements", e,
						getScreenshot());
			}
		} else {
			// logging
			log(Level.ERROR, "Find child web elements",
					"One or more of the required fields is null. parentWebElement: {}, by: {}", parentWebElement, by);
			// reporting
			reportStepFailure("Find child web elements",
					String.format("One or more of the required fields is null. parentWebElement: %s, by: %s",
							parentWebElement, by),
					null);
		}
	...
	/**
	 * Get list of locators from the given 'locatorString' string where each locator
	 * key and value pair is separated by ';'.
	 * <p>
	 * Example : {@code xpath~//div[@class='some-class'];xpath~//div[@id='some-id']}
	 * </p>
	 * 
	...
	private By getLocator(String locatorKey, String locatorValue) {
		if (StringUtils.isNotBlank(locatorKey) && StringUtils.isNotBlank(locatorValue)) {
			...
			// logging
			log(Level.ERROR, null, "One or more required fields is blank. locatorKey: {}, locatorValue: {}", locatorKey,
					locatorValue);
			// reporting
	 ...
	 * <p>
	 * Example : {@code xpath~//div[@class='some-class']}
	 * </p>
	 * 
	 * @param locatorPairString string containing the locator key and value
	 *                          separated by '~'
	 ...
	 * 
	 * @param enableLogging boolean value to determine whether logging will be done
	 */
	 ...
	 * @param arguments       a list (actually an array) of arguments to be used
	 *                        with the information to log
	 */
	private void log(Level level, String stepDescription, String details, Object... arguments) {
		if (enableLogging) {
			details = StringUtils.isNotBlank(stepDescription) ? String.format("%s: %s", stepDescription, details)
					: details;
	...
	public class ReportingConfigurer {
		...
		/**
		 * Gets the boolean value that determines whether report generation is enabled
		 * or not.
		 * 
		 * @return the boolean value that determines whether report generation is
		 *         enabled or not
		 */
		 ...
		 * 
		 * @param enable boolean value to determine whether screenshots for successful
		 *               steps will be attached in the reports
		 */
		...
		/**
		 * Gets the boolean value that determines whether attaching screenshots for
		 * successful steps is enabled or not.
		 * 
		 * @return the boolean value that determines whether attaching screenshots for
		 *         successful steps is enabled or not
		 */
		 ...
		 * 
		 * @param enable boolean value to determine whether screenshots for failed steps
		 *               will be attached in the reports
		 */
		...
		/**
		 * Gets the boolean value that determines whether attaching screenshots for
		 * failed steps is enabled or not.
		 * 
		 * @return the boolean value that determines whether attaching screenshots for
		 *         failed steps is enabled or not
		 */
	...
	private void reportStepSuccess(String stepDescription, String details, byte[] screenshot) {
		if (enableReporting) {
			if (enableSuccessScreenshots && ArrayUtils.isNotEmpty(screenshot)) {
                ReportingUtility.reportStepSuccess(stepDescription, details, screenshot);
			} else {
            	ReportingUtility.reportStepSuccess(stepDescription, details);
			}
		}
	}
	...
	private void reportStepFailure(String stepDescription, String details, byte[] screenshot) {
		if (enableReporting) {
			if (enableFailureScreenshots && ArrayUtils.isNotEmpty(screenshot)) {
            	ReportingUtility.reportStepFailure(stepDescription, details, screenshot);
			} else {
            	ReportingUtility.reportStepFailure(stepDescription, details);
			}
		}
	}
	...
	private void reportStepFailure(String stepDescription, String details, Throwable exception, byte[] screenshot) {
		if (enableReporting) {
			if (enableFailureScreenshots && ArrayUtils.isNotEmpty(screenshot)) {
            	ReportingUtility.reportStepFailure(stepDescription, details, exception, screenshot);
			} else {
            	ReportingUtility.reportStepFailure(stepDescription, details, exception);
			}
		}
	}
}
